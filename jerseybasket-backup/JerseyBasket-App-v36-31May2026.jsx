 ...
 * Version:   v36
 * Updated:   31 May 2026
 * Changes:   Store on/off toggle added. Users can hide any store via ⚙️ Settings
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#050d1a 0%,#0b1c35 55%,#061220 100%)", fontFamily:"'Georgia',serif", color:"#f0f4f8" }}>
      ...
      <header style={{ position:"sticky",top:0,zIndex:100, background:"rgba(5,13,26,.96)",backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,255,255,.07)",padding:"0 16px" }}>
        <div style={{ maxWidth:1040,margin:"0 auto" }}>
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",paddingTop:11,paddingBottom:8,gap:8 }}>
            <div style={{ minWidth:0,flexShrink:1 }}>
              <div style={{ display:"flex",alignItems:"center",gap:9 }}>
                <span style={{ fontSize:22 }}>🛍️</span>
                <div>
                  <div style={{ fontSize:17,fontWeight:700,letterSpacing:"-0.5px",lineHeight:1.1,whiteSpace:"nowrap" }}>
            ...
            <div style={{ display:"flex",gap:5,alignItems:"center",flexShrink:0 }}>
              ...
              style={{ background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.1)",borderRadius:9,width:30,height:30,color:"#94a3b8",cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",transition:"all .2s",flexShrink:0 }}
              ...
              title="Settings — show/hide stores"
              style={{ background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.1)",borderRadius:9,width:30,height:30,color:"#94a3b8",cursor:"pointer",fontSize:15,display:"flex",alignItems:"center",justifyContent:"center",transition:"all .2s" }}
              onMouseEnter={e=>{e.currentTarget.style.background="rgba(251,191,36,.15)";e.currentTarget.style.borderColor="rgba(251,191,36,.35)";e.currentTarget.style.color="#fcd34d";}}
              onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,.08)";e.currentTarget.style.borderColor="rgba(255,255,255,.1)";e.currentTarget.style.color="#94a3b8";}}>
                ⚙️
              </button>
            </div>
          </div>
          {/* ── SECOND ROW: nav tabs + Add/Favourites/Basket ── */}
          <div style={{ display:"flex",alignItems:"center",gap:4,paddingBottom:9,justifyContent:"space-between" }}>
            <div style={{ display:"flex",gap:3,flexShrink:1,overflow:"hidden" }}>
              {[["shop","🛒 Shop"],["basket","🧺 Basket"],["favourites",`♥ Saved${favCount>0?" ("+favCount+")":""}`],["compare","📊 Stores"]].map(([v,lbl])=>(
                <button key={v} onClick={()=>setView(v)} style={{ background:view===v?(v==="favourites"?"rgba(244,63,94,.16)":"rgba(34,197,94,.16)"):"transparent", border:view===v?(v==="favourites"?"1px solid rgba(244,63,94,.38)":"1px solid rgba(34,197,94,.38)"):"1px solid transparent", color:view===v?(v==="favourites"?"#fb7185":"#22c55e"):"#64748b", borderRadius:7,padding:"4px 10px",cursor:"pointer",fontSize:11,fontWeight:700,whiteSpace:"nowrap" }}>{lbl}</button>
              ))}
            </div>
            <div style={{ display:"flex",gap:5,alignItems:"center",flexShrink:0 }}>
              {/* Add Item */}
              <button onClick={()=>setShowAddModal(true)} style={{ background:"rgba(34,197,94,.12)",border:"1px solid rgba(34,197,94,.3)",color:"#22c55e",borderRadius:9,padding:"5px 10px",cursor:"pointer",fontSize:11,fontWeight:700,whiteSpace:"nowrap" }}>+ Add</button>
              {/* Favourites */}
              <button onClick={()=>setView("favourites")} style={{ background:view==="favourites"?"rgba(244,63,94,.2)":favCount>0?"rgba(244,63,94,.1)":"rgba(255,255,255,.08)",border:`1px solid ${view==="favourites"||favCount>0?"rgba(244,63,94,.4)":"rgba(255,255,255,.1)"}`,borderRadius:9,width:30,height:30,color:view==="favourites"?"#fb7185":favCount>0?"#f43f5e":"#64748b",cursor:"pointer",fontSize:13,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:4,transition:"all .2s",position:"relative" }}>
                {view==="favourites"?"♥":"♡"}
                {favCount>0&&<span style={{ position:"absolute",top:-4,right:-4,background:"#f43f5e",color:"#fff",borderRadius:"50%",width:14,height:14,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700 }}>{favCount}</span>}
              </button>
              {/* Basket */}
              <button onClick={()=>setView(view==="basket"?"shop":"basket")} style={{ background:basketCount>0?"linear-gradient(135deg,#16a34a,#15803d)":"rgba(255,255,255,.08)",border:"none",borderRadius:9,width:30,height:30,color:"#fff",cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",transition:"all .2s" }}>
                🧺{basketCount>0&&<span style={{ position:"absolute",top:-4,right:-4,background:"#22c55e",color:"#fff",borderRadius:"50%",width:14,height:14,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700 }}>{basketCount}</span>}
              </button>
            </div>
          </div>
        </div>
