 ...
 * Version:   v23
 * Updated:   22 May 2026
 * Changes:   Fixed banner click on desktop — moved onClick to outer
 *            wrapper so it fires reliably regardless of slide animation
 * ============================================================
 */
...
const BASE_PRODUCTS = [
  ...
  /* ── DAIRY & EGGS ─────────────────────────────────────────────── */
  {id:1,  name:"Full Fat Milk (2L)",           cat:"🥛 Dairy & Eggs", icon:"🥛", prices:sp(1.75,[0,0.14,0.35,0.24,0.07])},
  {id:2,  name:"Semi-Skimmed Milk (2L)",       cat:"🥛 Dairy & Eggs", icon:"🥛", prices:sp(1.70,[0,0.14,0.34,0.24,0.07])},
  ...
  {id:10, name:"Cheddar Mature (400g)",        cat:"🥛 Dairy & Eggs", icon:"🧀", prices:sp(2.80,[0,0.15,0.6,0.4,0.07])},
  ...
  {id:14, name:"Parmesan (100g)",              cat:"🥛 Dairy & Eggs", icon:"🧀", prices:sp(2.50,[0,0.25,0.95,0.75,0.12])},
  {id:15, name:"Feta Cheese (200g)",           cat:"🥛 Dairy & Eggs", icon:"🧀", prices:sp(2.00,[0,0.2,0.8,0.6,0.1])},
  {id:16, name:"Butter Salted (250g)",         cat:"🥛 Dairy & Eggs", icon:"🧈", prices:sp(2.20,[0,0.15,0.6,0.45,0.07])},
  {id:17, name:"Butter Unsalted (250g)",       cat:"🥛 Dairy & Eggs", icon:"🧈", prices:sp(2.25,[0,0.15,0.6,0.45,0.07])},
  {id:18, name:"Spreadable Butter (500g)",     cat:"🥛 Dairy & Eggs", icon:"🧈", prices:sp(3.10,[0,0.22,0.85,0.65,0.11])},
  {id:19, name:"Greek Yoghurt (500g)",         cat:"🥛 Dairy & Eggs", icon:"🫙", prices:sp(1.95,[0,0.15,0.65,0.45,0.07])},
  ...
  {id:26, name:"Cottage Cheese (300g)",        cat:"🥛 Dairy & Eggs", icon:"🫙", prices:sp(1.50,[0,0.15,0.6,0.45,0.07])},
  {id:27, name:"Cream Cheese (200g)",          cat:"🥛 Dairy & Eggs", icon:"🧀", prices:sp(1.80,[0,0.18,0.7,0.52,0.09])},
  {id:28, name:"Clotted Cream (113g)",         cat:"🥛 Dairy & Eggs", icon:"🍦", prices:sp(2.00,[0,0.22,0.8,0.6,0.11])},

  /* ── BREAD & BAKERY ───────────────────────────────────────────── */
  {id:29, name:"White Sliced Bread 800g",      cat:"🍞 Bread & Bakery",icon:"🍞", prices:sp(1.25,[0,0.1,0.55,0.4,0.05])},
  {id:30, name:"Wholemeal Bread 800g",         cat:"🍞 Bread & Bakery",icon:"🥖", prices:sp(1.45,[0,0.1,0.55,0.4,0.05])},
  {id:31, name:"Seeded Batch Loaf",            cat:"🍞 Bread & Bakery",icon:"🥖", prices:sp(1.75,[0,0.15,0.65,0.5,0.07])},
  ...
  {id:37, name:"Pitta Bread (6pk)",            cat:"🍞 Bread & Bakery",icon:"🫓", prices:sp(1.20,[0,0.15,0.65,0.5,0.07])},
  {id:38, name:"Wraps / Tortillas (8pk)",      cat:"🍞 Bread & Bakery",icon:"🫓", prices:sp(1.60,[0,0.15,0.7,0.55,0.07])},
  {id:39, name:"Naan Bread (4pk)",             cat:"🍞 Bread & Bakery",icon:"🫓", prices:sp(1.50,[0,0.15,0.65,0.5,0.07])},
  ...
  {id:44, name:"Crumpets (6pk)",               cat:"🍞 Bread & Bakery",icon:"🫓", prices:sp(1.10,[0,0.12,0.48,0.36,0.06])},
  ...
  {id:50, name:"Chicken Drumsticks (1kg)",     cat:"🥩 Meat & Fish",   icon:"🍗", prices:sp(2.80,[0,0.18,0.8,0.58,0.09])},
  {id:51, name:"Whole Chicken (~1.4kg)",       cat:"🥩 Meat & Fish",   icon:"🍗", prices:sp(5.50,[0,0.4,1.8,1.3,0.2])},
  {id:52, name:"Minced Beef (500g)",           cat:"🥩 Meat & Fish",   icon:"🥩", prices:sp(3.50,[0,0.15,0.9,0.7,0.07])},
  ...
  {id:61, name:"Back Bacon (200g)",            cat:"🥩 Meat & Fish",   icon:"🥓", prices:sp(2.50,[0,0.15,0.7,0.5,0.07])},
  {id:62, name:"Streaky Bacon (200g)",         cat:"🥩 Meat & Fish",   icon:"🥓", prices:sp(2.30,[0,0.14,0.65,0.47,0.07])},
  {id:63, name:"Sausages Pork (8pk)",          cat:"🥩 Meat & Fish",   icon:"🌭", prices:sp(2.80,[0,0.2,0.8,0.6,0.1])},
  ...
  {id:67, name:"Ham Slices (120g)",            cat:"🥩 Meat & Fish",   icon:"🥩", prices:sp(1.80,[0,0.15,0.65,0.5,0.07])},
  {id:68, name:"Turkey Mince (500g)",          cat:"🥩 Meat & Fish",   icon:"🦃", prices:sp(3.40,[0,0.22,0.95,0.72,0.11])},
  {id:69, name:"Salmon Fillets (2pk)",         cat:"🥩 Meat & Fish",   icon:"🐟", prices:sp(4.20,[0,0.25,1.3,1,0.12])},
  {id:70, name:"Smoked Salmon (100g)",         cat:"🥩 Meat & Fish",   icon:"🐟", prices:sp(3.20,[0,0.3,1.2,0.95,0.15])},
  ...
  {id:81, name:"Apples Braeburn (6pk)",        cat:"🥦 Fruit & Veg",  icon:"🍎", prices:sp(1.40,[0,0.15,0.55,0.4,0.07])},
  ...
  /* ── FROZEN ───────────────────────────────────────────────────── */
  {id:128,name:"Frozen Peas (900g)",           cat:"🧊 Frozen",       icon:"🟢", prices:sp(1.80,[0,0.18,0.7,0.55,0.09])},
  {id:129,name:"Frozen Peas & Sweetcorn (1kg)",cat:"🧊 Frozen",       icon:"🟢", prices:sp(1.60,[0,0.16,0.65,0.5,0.08])},
  {id:130,name:"Frozen Chips Straight (1.5kg)",cat:"🧊 Frozen",       icon:"🍟", prices:sp(2.20,[0,0.2,0.8,0.6,0.1])},
  {id:131,name:"Frozen Chips Crinkle (1kg)",   cat:"🧊 Frozen",       icon:"🍟", prices:sp(1.90,[0,0.18,0.72,0.55,0.09])},
  {id:132,name:"Frozen Oven Pizza (Margherita)",cat:"🧊 Frozen",      icon:"🍕", prices:sp(2.50,[0,0.25,0.95,0.75,0.12])},
  {id:133,name:"Frozen Oven Pizza (Pepperoni)", cat:"🧊 Frozen",      icon:"🍕", prices:sp(2.80,[0,0.28,1,0.8,0.14])},
  {id:134,name:"Ice Cream Vanilla (1L)",       cat:"🧊 Frozen",       icon:"🍨", prices:sp(3.00,[0,0.3,1.1,0.85,0.15])},
  {id:135,name:"Ice Cream Tub Luxury (500ml)", cat:"🧊 Frozen",       icon:"🍨", prices:sp(3.80,[0,0.38,1.35,1.05,0.19])},
  {id:136,name:"Frozen Fish Fingers (12pk)",   cat:"🧊 Frozen",       icon:"🐟", prices:sp(2.80,[0,0.25,1,0.8,0.12])},
  ...
  {id:144,name:"Frozen Garlic Bread (2pk)",    cat:"🧊 Frozen",       icon:"🍞", prices:sp(1.50,[0,0.15,0.6,0.46,0.07])},
  ...
  {id:169,name:"Red Wine Bottle (75cl)",       cat:"🥤 Drinks",       icon:"🍷", prices:sp(6.50,[0,0.5,2,1.6,0.25])},
  {id:170,name:"White Wine Bottle (75cl)",     cat:"🥤 Drinks",       icon:"🍾", prices:sp(6.00,[0,0.5,2,1.6,0.25])},
  {id:171,name:"Rosé Wine Bottle (75cl)",      cat:"🥤 Drinks",       icon:"🌸", prices:sp(6.20,[0,0.52,2.05,1.62,0.26])},
  {id:172,name:"Prosecco (75cl)",              cat:"🥤 Drinks",       icon:"🥂", prices:sp(8.00,[0,0.7,2.5,2,0.35])},
  {id:173,name:"Beer Lager 4pk (440ml)",       cat:"🥤 Drinks",       icon:"🍺", prices:sp(4.20,[0,0.4,1.5,1.2,0.2])},
  ...
  {id:176,name:"Vodka (70cl)",                 cat:"🥤 Drinks",       icon:"🍸", prices:sp(15.0,[0,1.2,4.5,3.5,0.6])},
  ...
  {id:179,name:"Squash (1L)",                  cat:"🥤 Drinks",       icon:"🥤", prices:sp(1.50,[0,0.15,0.6,0.46,0.07])},

  /* ── PANTRY ───────────────────────────────────────────────────── */
  {id:180,name:"Pasta Penne (500g)",           cat:"🍝 Pantry",       icon:"🍝", prices:sp(0.89,[0,0.06,0.41,0.26,0.03])},
  {id:181,name:"Spaghetti (500g)",             cat:"🍝 Pantry",       icon:"🍝", prices:sp(0.89,[0,0.06,0.41,0.26,0.03])},
  {id:182,name:"Fusilli (500g)",               cat:"🍝 Pantry",       icon:"🍝", prices:sp(0.89,[0,0.06,0.41,0.26,0.03])},
  {id:183,name:"Tagliatelle (500g)",           cat:"🍝 Pantry",       icon:"🍝", prices:sp(0.95,[0,0.08,0.42,0.28,0.04])},
  {id:184,name:"Tinned Tomatoes (400g)",       cat:"🍝 Pantry",       icon:"🍅", prices:sp(0.65,[0,0.05,0.34,0.25,0.03])},
  ...
  {id:190,name:"Tinned Baked Beans (415g)",    cat:"🍝 Pantry",       icon:"🫘", prices:sp(0.60,[0,0.06,0.35,0.25,0.03])},
  ...
  {id:194,name:"Sunflower Oil (1L)",           cat:"🍝 Pantry",       icon:"🌻", prices:sp(2.20,[0,0.18,0.9,0.7,0.09])},
  {id:195,name:"Vegetable Oil (1L)",           cat:"🍝 Pantry",       icon:"🌻", prices:sp(1.90,[0,0.16,0.78,0.6,0.08])},
  {id:196,name:"Basmati Rice (1kg)",           cat:"🍝 Pantry",       icon:"🍚", prices:sp(1.80,[0,0.15,0.7,0.5,0.07])},
  ...
  {id:199,name:"Porridge Oats (500g)",         cat:"🍝 Pantry",       icon:"🌾", prices:sp(1.30,[0,0.12,0.5,0.38,0.06])},
  ...
  {id:205,name:"Plain Flour (1.5kg)",          cat:"🍝 Pantry",       icon:"🌾", prices:sp(1.20,[0,0.12,0.5,0.38,0.06])},
  {id:206,name:"Self Raising Flour (1.5kg)",   cat:"🍝 Pantry",       icon:"🌾", prices:sp(1.25,[0,0.12,0.5,0.38,0.06])},
  {id:207,name:"Sugar White (1kg)",            cat:"🍝 Pantry",       icon:"🍬", prices:sp(1.00,[0,0.1,0.38,0.28,0.05])},
  {id:208,name:"Caster Sugar (1kg)",           cat:"🍝 Pantry",       icon:"🍬", prices:sp(1.10,[0,0.1,0.4,0.3,0.05])},
  {id:209,name:"Icing Sugar (500g)",           cat:"🍝 Pantry",       icon:"🍬", prices:sp(0.90,[0,0.09,0.35,0.26,0.04])},
  ...
  {id:212,name:"Brown Sauce (400ml)",          cat:"🍝 Pantry",       icon:"🥄", prices:sp(1.60,[0,0.16,0.65,0.5,0.08])},
  ...
  {id:219,name:"Stock Cubes Beef (12pk)",      cat:"🍝 Pantry",       icon:"🫙", prices:sp(1.20,[0,0.12,0.48,0.36,0.06])},
  ...
  {id:222,name:"Honey (340g)",                 cat:"🍝 Pantry",       icon:"🍯", prices:sp(2.80,[0,0.25,1,0.8,0.12])},
  {id:223,name:"Peanut Butter Smooth (340g)",  cat:"🍝 Pantry",       icon:"🥜", prices:sp(2.50,[0,0.22,0.88,0.68,0.11])},
  {id:224,name:"Peanut Butter Crunchy (340g)", cat:"🍝 Pantry",       icon:"🥜", prices:sp(2.50,[0,0.22,0.88,0.68,0.11])},
  {id:225,name:"Strawberry Jam (454g)",        cat:"🍝 Pantry",       icon:"🍓", prices:sp(1.80,[0,0.18,0.7,0.55,0.09])},
  {id:226,name:"Marmalade (454g)",             cat:"🍝 Pantry",       icon:"🍊", prices:sp(1.80,[0,0.18,0.7,0.55,0.09])},
  ...
  {id:237,name:"Cornflour (250g)",             cat:"🍝 Pantry",       icon:"🌾", prices:sp(0.80,[0,0.08,0.3,0.22,0.04])},
  ...
  {id:240,name:"Crisps Large Sharing Bag",     cat:"🥨 Snacks & Treats",icon:"🥨",prices:sp(2.20,[0,0.22,0.85,0.65,0.11])},
  ...
  {id:261,name:"Kettle Chips (150g)",          cat:"🥨 Snacks & Treats",icon:"🥨",prices:sp(1.80,[0,0.18,0.7,0.54,0.09])},
  {id:262,name:"Pretzels (175g)",              cat:"🥨 Snacks & Treats",icon:"🥨",prices:sp(1.60,[0,0.16,0.64,0.49,0.08])},

  /* ── HOUSEHOLD ────────────────────────────────────────────────── */
  {id:263,name:"Washing Up Liquid (500ml)",    cat:"🧹 Household",    icon:"🧴", prices:sp(1.20,[0,0.15,0.55,0.4,0.07])},
  {id:264,name:"Washing Up Liquid (900ml)",    cat:"🧹 Household",    icon:"🧴", prices:sp(2.00,[0,0.22,0.85,0.65,0.11])},
  {id:265,name:"Laundry Liquid (800ml)",       cat:"🧹 Household",    icon:"🫧", prices:sp(5.50,[0,0.5,1.8,1.4,0.25])},
  {id:266,name:"Laundry Tablets (30pk)",       cat:"🧹 Household",    icon:"🫧", prices:sp(7.50,[0,0.7,2.5,2,0.35])},
  {id:267,name:"Laundry Capsules (32pk)",      cat:"🧹 Household",    icon:"🫧", prices:sp(8.50,[0,0.8,2.8,2.2,0.4])},
  {id:268,name:"Fabric Conditioner (1L)",      cat:"🧹 Household",    icon:"🫧", prices:sp(3.80,[0,0.35,1.3,1,0.17])},
  {id:269,name:"Toilet Rolls (9pk)",           cat:"🧹 Household",    icon:"🧻", prices:sp(4.50,[0,0.4,1.6,1.2,0.2])},
  {id:270,name:"Toilet Rolls (24pk)",          cat:"🧹 Household",    icon:"🧻", prices:sp(10.0,[0,0.88,3.5,2.7,0.44])},
  {id:271,name:"Kitchen Roll (2pk)",           cat:"🧹 Household",    icon:"🧻", prices:sp(1.80,[0,0.18,0.7,0.55,0.09])},
  ...
  {id:277,name:"All-Purpose Spray (500ml)",    cat:"🧹 Household",    icon:"🧹", prices:sp(1.90,[0,0.18,0.72,0.55,0.09])},
  {id:278,name:"Bleach (750ml)",               cat:"🧹 Household",    icon:"🧹", prices:sp(1.10,[0,0.11,0.44,0.33,0.06])},
  ...
  {id:286,name:"Batteries AA (4pk)",           cat:"🧹 Household",    icon:"🔋", prices:sp(3.50,[0,0.32,1.2,0.95,0.16])},
  {id:287,name:"Batteries AAA (4pk)",          cat:"🧹 Household",    icon:"🔋", prices:sp(3.50,[0,0.32,1.2,0.95,0.16])},
  {id:288,name:"Candles (4pk)",                cat:"🧹 Household",    icon:"🕯️", prices:sp(2.20,[0,0.22,0.85,0.65,0.11])},

  /* ── HEALTH & BEAUTY ──────────────────────────────────────────── */
  {id:289,name:"Shampoo (400ml)",              cat:"💊 Health & Beauty",icon:"🧴",prices:sp(2.50,[0,0.25,0.95,0.75,0.12])},
  {id:290,name:"Conditioner (400ml)",          cat:"💊 Health & Beauty",icon:"🧴",prices:sp(2.50,[0,0.25,0.95,0.75,0.12])},
  {id:291,name:"2-in-1 Shampoo (400ml)",       cat:"💊 Health & Beauty",icon:"🧴",prices:sp(2.80,[0,0.28,1,0.8,0.14])},
  {id:292,name:"Shower Gel (500ml)",           cat:"💊 Health & Beauty",icon:"🚿",prices:sp(2.00,[0,0.2,0.8,0.6,0.1])},
  ...
  {id:295,name:"Hand Soap (250ml)",            cat:"💊 Health & Beauty",icon:"🧼",prices:sp(1.40,[0,0.15,0.55,0.42,0.07])},
  ...
  {id:299,name:"Toothpaste (75ml)",            cat:"💊 Health & Beauty",icon:"🪥",prices:sp(1.80,[0,0.18,0.7,0.55,0.09])},
  {id:300,name:"Toothpaste Electric Compat",   cat:"💊 Health & Beauty",icon:"🪥",prices:sp(2.50,[0,0.25,0.95,0.75,0.12])},
  {id:301,name:"Toothbrush Manual (2pk)",      cat:"💊 Health & Beauty",icon:"🪥",prices:sp(2.20,[0,0.22,0.85,0.65,0.11])},
  {id:302,name:"Dental Floss (50m)",           cat:"💊 Health & Beauty",icon:"🦷",prices:sp(1.50,[0,0.15,0.6,0.46,0.07])},
  {id:303,name:"Mouthwash (500ml)",            cat:"💊 Health & Beauty",icon:"🦷",prices:sp(2.50,[0,0.25,0.95,0.75,0.12])},
  ...
  {id:310,name:"Ibuprofen (16pk)",             cat:"💊 Health & Beauty",icon:"💊",prices:sp(1.10,[0,0.12,0.45,0.34,0.06])},
  {id:311,name:"Cold & Flu Tablets (16pk)",    cat:"💊 Health & Beauty",icon:"💊",prices:sp(3.50,[0,0.32,1.2,0.95,0.16])},
  {id:312,name:"Vitamin C (60pk)",             cat:"💊 Health & Beauty",icon:"💊",prices:sp(4.50,[0,0.4,1.6,1.2,0.2])},
  ...
  {id:317,name:"Feminine Pads (12pk)",         cat:"💊 Health & Beauty",icon:"🩸",prices:sp(2.80,[0,0.28,1,0.8,0.14])},
  {id:318,name:"Tampons (16pk)",               cat:"💊 Health & Beauty",icon:"🩸",prices:sp(2.80,[0,0.28,1,0.8,0.14])},
  {id:319,name:"Condoms (12pk)",               cat:"💊 Health & Beauty",icon:"🩺",prices:sp(6.50,[0,0.6,2.2,1.8,0.3])},

  /* ── BABY & CHILD ─────────────────────────────────────────────── */
  {id:320,name:"Nappies Size 3 (40pk)",        cat:"🍼 Baby & Child",  icon:"🍼",prices:sp(7.50,[0,0.68,2.5,2,0.34])},
  {id:321,name:"Nappies Size 4 (36pk)",        cat:"🍼 Baby & Child",  icon:"🍼",prices:sp(7.80,[0,0.7,2.6,2.1,0.35])},
  {id:322,name:"Nappies Size 5 (34pk)",        cat:"🍼 Baby & Child",  icon:"🍼",prices:sp(8.00,[0,0.72,2.65,2.1,0.36])},
  {id:323,name:"Baby Wipes (72pk)",            cat:"🍼 Baby & Child",  icon:"🧻",prices:sp(1.80,[0,0.18,0.7,0.54,0.09])},
  {id:324,name:"Baby Wipes (3pk)",             cat:"🍼 Baby & Child",  icon:"🧻",prices:sp(4.50,[0,0.42,1.6,1.2,0.21])},
  {id:325,name:"Baby Shampoo (300ml)",         cat:"🍼 Baby & Child",  icon:"🧴",prices:sp(3.00,[0,0.28,1.1,0.85,0.14])},
  ...
  /* ── PET CARE ─────────────────────────────────────────────────── */
  {id:333,name:"Dog Food Dry (2kg)",           cat:"🐾 Pet Care",      icon:"🐶",prices:sp(8.00,[0,0.72,2.65,2.1,0.36])},
  ...
  {id:336,name:"Cat Food Dry (2kg)",           cat:"🐾 Pet Care",      icon:"🐱",prices:sp(7.50,[0,0.68,2.5,2,0.34])},
  {id:337,name:"Cat Food Wet Pouches (12pk)",  cat:"🐾 Pet Care",      icon:"🐱",prices:sp(5.50,[0,0.5,1.8,1.4,0.25])},
            ...
            <div style={{ display:"flex",gap:9,marginBottom:11,flexWrap:"wrap" }}>
              <div style={{ flex:1,minWidth:160,position:"relative" }}>
                ...
                <input
                  ...
                  style={{ width:"100%",padding:"8px 32px 8px 34px",background:"rgba(255,255,255,.06)",border:`1px solid ${searchQuery?"rgba(34,197,94,.4)":"rgba(255,255,255,.10)"}`,borderRadius:9,color:"#fff",fontSize:12,outline:"none",boxSizing:"border-box",transition:"border-color .2s" }}
                />
                {searchQuery && (
                  <button
              ...
              <div>
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:6,marginBottom:14 }}>
                  {basketItems.map(item=>{
                    ...
                    return(
                      <div key={item.key} style={{ display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(255,255,255,.04)",borderRadius:12,padding:"9px 13px",border:"1px solid rgba(255,255,255,.07)" }}>
                        ...
                        <div style={{ display:"flex",alignItems:"center",gap:7,flexShrink:0 }}>
                          <div style={{ display:"flex",alignItems:"center",gap:5,background:"rgba(255,255,255,.06)",borderRadius:8,padding:"3px 7px" }}>
                            <button onClick={()=>removeFromBasket(item.key)} style={{ background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:13,fontWeight:700,lineHeight:1 }}>−</button>
                            <span style={{ fontSize:11,fontWeight:700,minWidth:13,textAlign:"center" }}>{item.qty}</span>
                            <button onClick={()=>addToBasket(item.product.id,item.store?.id)} style={{ background:"none",border:"none",color:"#22c55e",cursor:"pointer",fontSize:13,fontWeight:700,lineHeight:1 }}>+</button>
                          </div>
                    ...
                    <div>
                      ...
                      <div style={{ display:"flex",flexDirection:"column",gap:6,marginBottom:14 }}>
                        {favBasketItems.map(item=>{
                          const overPay=item.price-getBestPrice(item.product);
                          return(
                            <div key={item.key} style={{ display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(244,63,94,.05)",borderRadius:12,padding:"9px 13px",border:"1px solid rgba(244,63,94,.15)" }}>
                              <div style={{ display:"flex",alignItems:"center",gap:8,flex:1,minWidth:0 }}>
                                <span style={{ fontSize:18,flexShrink:0 }}>{item.product.icon}</span>
                                <div style={{ minWidth:0 }}>
                                  <Tooltip text={item.product.name}>
